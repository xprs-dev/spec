...
6. [Signing and privacy](#6-signing-and-privacy)

**Part II. Messaging**

7. [Messages](#7-messages)
8. [Asking and answering](#8-asking-and-answering)
9. [Relaying and carried messages](#9-relaying-and-carried-messages)
10. [Closed groups](#10-closed-groups)
11. [Commands](#11-commands)

**Part III. Archiving and services**

12. [Archiver](#12-archiver)
13. [Services](#13-services)
14. [Channels](#14-channels)
15. [Observations](#15-observations)

**Part IV. Posting and status**

16. [Status](#16-status)
17. [Polls](#17-polls)
18. [Blog posts](#18-blog-posts)
19. [Passages](#19-passages)
...
21. [Tracks](#21-tracks)
22. [Places](#22-places)
23. [Calls for help](#23-calls-for-help)
24. [Warnings](#24-warnings)
25. [Notices](#25-notices)
26. [Events](#26-events)
27. [Offers and needs](#27-offers-and-needs)

**Part VI. Identity and operating rules**

28. [Reserved words](#28-reserved-words)
29. [Proving a callsign](#29-proving-a-callsign)
30. [Airtime](#30-airtime)
31. [Adding a field, worked](#31-adding-a-field-worked)
32. [Operating alongside APRS](#32-operating-alongside-aprs)

**Part VII. Reference**

33. [Examples](#33-examples)
34. [Worked exchanges](#34-worked-exchanges)
...
The rules every field obeys, where a callsign comes from, what a packet looks like on the wire, how it is named, and how it is signed.
...
- **Identity was asserted, never proven.** Any station could transmit any
  ...
  checkable (section 6), and a licensed callsign can be bound to a key and
  proven live on air (section 29).
...
, stations carry mail for the absent and hand it over with signed receipts
  (section 9), and history, so a station away for four days asks for the
  days it missed (section 33.2).
- **The internet side was one central system.** APRS-IS sees every packet,
  ...
  traffic (section 12), no archiver holds the network, every archiver can
  point across it.
- **Content stopped at 67 characters.** XPRS carries files of any size by
  ...
  (section 7.7), with the packet layer never carrying a byte it cannot
  afford.
- **One band, one plan.** XPRS is bearer-neutral: LoRa, Bluetooth, WiFi,
  ...
  airtime arithmetic (section 30), with a documented way for a pair to move
  their long business off the shared channel (section 14.7).
...
two meet under APRS rules where they meet (sections 7.4 and 32).
...
| **relay** (APRS: digipeater) | repeats a packet on the medium it heard it, within the hop budget, appending itself to `via:` | 9.1-13.2 |
| **carrier** | holds a message in custody for a station that is absent, and hands it over, with a signed receipt, when it can | 9.3-13.7 |
| **gateway** (APRS: iGate) | republishes packets onto something that is not XPRS, an archiver, APRS-IS, the internet, under the scope rules, and publishes who it hears so the absent stay reachable | 9.11.3, 12.1, 12.8 |
| **file server** | holds content-addressed files and serves them by hash (`serve:files`, `cmd:file`, `q:have`) | 7.7, 24 |
| **archiver** | keeps packets and answers for them: the spool of what it heard (re-aired on `cmd:history`), the publications of its depositors, and mail held for recipients (`t:mailbox hold:` names one deliberately). Every station is one at its own scale; `serve:archive` announces the role to others | 24, 11.2, 36 |
| **group** | an address several stations read, a name, not a boundary, and not a station | 7.3, 26 |
...
it advertises with `serve:` (section 13), and what it is obliged to do is
nothing (section 30.3), every role here is volunteered and bounded by its
operator's budgets.
...
X4 = automated device, operated by a controller (section 11.7.1)
X5 = group (section 10)
```
...
callsign and announces from where it now stands, and `site:` (section 14)
...
commanded (section 11.9), an `X2` does as an `X3` does; the prefix states
...
identity verifies a signature against the full public key (section 6). No
...
condition of the licence. Section 6.4.1 states the rule and section 6.4.2 shows
...
It learns it is not alone the moment it hears a beacon (section 15.6) carrying
...
meaningless, a person is not a door, so a command (section 11) names a
device:
...
(section 9.7.1) names the device that sent it:
...
Mail held for a bare callsign (section 6.3) is discharged by handing it to
...
| `q` | `words` | what the sender wants back (section 8) |
| `s` | `words` | what this packet answers or reports (section 8) |
...
| `urg` | `enum` | how much this is worth carrying (section 9.5) |
| `scope` | `scope` | how far this may be relayed, default global (section 9.11) |
| `lang` | `lang` | language of `m:`, default English (section 4.7) |
| `hold` | `path` | preferred mailboxes, in order (section 9.12) |
| `serve` | `words` | what a station does for others (section 13) |
| `cmd` | `label` | the action a command asks for (section 11) |
...
| `relay` | `path` | callsigns the sender asks to relay this packet, in order (section 9.2.2) |
| `route` | `path` | the route a receipt is acknowledging (section 9.10) |
| `add` | `enum` | something this packet adds (section 7.5) |
| `remove` | `enum` | something this packet withdraws (section 7.5) |
| `grant` | `path` | callsigns admitted to a group (section 10) |
| `revoke` | `path` | callsigns removed or suspended (section 10) |
| `role` | `enum` | what a grant confers: `mod`, `sub`, or absent for a member |
| `hide` | `enum` | what a moderator withdraws from view: `message` |
| `mood` | `enum` | how the sender feels (section 16.1) |
| `only` | `call` | narrows a replay to one callsign or group (section 33.2) |
| `opt` | `labels` | the choices in a poll, two to six (section 17) |
| `root` | `hex6` | the packet a thread hangs from (section 7.4) |
| `size` | `qty` | how large a file is (section 7.7.1) |
| `vote` | `label` | the option chosen in a poll (section 17.3) |
| `via` | `path` | callsigns that relayed this packet, oldest first (section 9) |
| `track` | `label` | name of a track this packet belongs to (section 21) |
| `title` | `label` | name of a post or event, stable across revisions |
| `dest` | `coord` | where a passage is bound (section 19) |
| `onboard` | `int` | how many people are aboard |
| `price` | `money` | what is being asked or offered (section 16.1) |
| `freq` | `qty` | a frequency (section 14) |
| `bw` | `qty` | bandwidth |
...
| `kind` | `enum` | nature of an event, values per packet type (sections 15, 16); in `cmd:history` the packet type asked for, or a comma-separated list of them (section 33.2) |
| `sev` | `enum` | severity of a warning (section 24) |
...
| `name` | `label` | filename, when the extension is not enough (section 7.7.1) |
| `ph` | `ref` | content hash of a file's piece list (section 7.7.2) |
...
| `track` | a point in a named track (section 21) |
| `sos` | a call for help (section 23) |
| `info` | a notice about conditions (section 25) |
| `blog` | a published post (section 18) |
| `poll` | a question put to everybody, with the choices (section 17) |
| `file` | what a file is, so it can be wanted (section 7.7.1) |
| `report` | a claim that a packet is spam or abuse (section 20) |
| `place` | somewhere useful that is not the sender (section 22) |
| `status` | a short post about the sender, now (section 16) |
| `passage` | where a vessel is going (section 19) |
| `event` | something happening at a time and place (section 26) |
| `offer` | what a station has (section 27) |
| `need` | what a station wants (section 27) |
| `channel` | a frequency a station uses (section 14) |
| `mailbox` | stations that hold mail for the sender (section 9.12) |
| `service` | what a station does for others (section 13) |
| `command` | asks a station to do something (section 11) |
| `result` | what happened to a command |
| `moderate` | an act of authority in a group (section 10) |
| `challenge` | a challenge to prove a callsign (section 29) |
| `response` | the answer to a challenge |
| `warning` | a warning about a hazard (section 24) |
| `ping` | a reachability test |
| `pong` | a reply to `ping` |

An unknown type is ignored. It is never an error and is never displayed as a
message. Types not listed here are reserved (section 26).
...
| `money` | an amount with an ISO 4217 code, optional leading `~` and `/` period, or one of `offers`, `swap`, `free` (section 16.2) | `~25EUR/day` |
| `qty` | a number followed immediately by its unit (section 15.9) | `48km/h` |
...
marks a redacted portion (section 6.2.1), and a reader strips it and reads
what remains.
...
- **A measurement carries its unit** (section 15.8). The number rules above
...
precision claimed (section 15.1). `temp:14.0` says the reading was measured to a
...
arrive in any order (section 7.6), so a warning carried once is a warning the
...
current. Two alternatives exist for stations without a clock (section 15.7).
...
**Every value is a JSON string.** `temp:14.2C` is `"21.2C"` and not `14.2`: the
...
packet as section 7.6 describes, `n:` removed, the `m:` values joined. A
...
a receipt, or a withdrawal of the sender's own earlier packet (section 11.2).
...
appends itself to `via:` (section 9), so a signature covering it would break at
...
section 7.3, because signing changes neither `f:`, `ts:` nor the payload.
...
### 6.1.1 When the signature does not fit

A signature is 65 bytes and the packet limit is 250, so a full observation can
run out of room. The weather station in section 33.3 is 193 bytes and would be
258 signed.
...
Where the payload cannot be trimmed, split it (section 7.6) and sign the last
...
characters are the identifier `399227` of section 7.3:
...
```cpp
// XPRS short-Schnorr sign over secp256k1 (section 6.1.2).
// Assumed primitives:
...
file (section 7.7) and redacts the same way; nothing new travels.
...
issued by an authority nothing in this format does: section 29 proves that the
key holder is present, and section 6.4.2 says where entitlement is checked
...
any other file in this format (section 7.7), fetched with `cmd:file` (section
11.2) if the receiver wants it and ignored entirely if it does not. A station
...
nothing (section 20.1). The decoration is larger and changes once a year, so it
...
### 6.4.1 Only an issued callsign may transmit on licensed spectrum
...
### 6.4.2 Associating an issued callsign with a key

An operator who wants their traffic to be verifiable under their real callsign
announces the binding themselves, with the identity packet of section 7.3 and
...
|---|---|
| does the holder of this key write these packets | the signature, section 7.1 |
| is that key holder present right now | a challenge, section 29, on licence-free spectrum only |
...
### 6.4.3 What signing costs on the amateur bands
...
- **A challenge cannot be put on amateur spectrum**, because the exchange in
  section 29 works by sealing a nonce. Prove a callsign on licence-free spectrum
...
- **Signing is authorship, not confidentiality.** On an amateur band XPRS can
  ...
  section 7.2 applies in full.

---

# Part II. Messaging

Saying things and asking things: one-to-one and group correspondence, how a packet is carried to someone who is not here, closed groups, and the command verbs.
...
callsign derived from it (section 10). Everything in this section applies to it
unchanged:
...
### 7.5.1 Replying, quoting and passing on

Three things a social network needs, and the format already had two of them.

**A reply is `r:`** (section 7.4), and it works on any packet carrying content.
...
heard the parent shows the comment and says so, which section 7.4 already
...
(section 11.2.1), and safe for the same reason: duplicates collapse on the
...
`t:receipt` with `s:ack` (section 33.5), which says a station heard it and is a
...
which `f:` and `track:` identify (section 21), and shows them once against the
...
(section 30.3). Being named is what makes a packet worth more than the traffic
...
- Only `m:` is split, with one exception: a packet carrying inline file bytes
  splits `b:` instead (section 7.7.4), and those parts are joined with nothing
...
- **A set is limited to 9 parts**, so `n:` is always three characters and the
  ...
  sent as a file (section 7.7) rather than split further: at that size the
...
**`size:` is the field that earns its bytes.** `cmd:file` (section 33.2) asks a
...
- `ph:`, a `ref`: the content hash of the file's **piece list** (7.7.2), which
...
splits like any packet (section 7.6), and a description is not beacon traffic.
...
### 7.7.2 Listings: a folder's files, and a file's pieces
...
say so (section 8), and may serve them.** That is the rule that turns a crowd
...
### 7.7.4 A small file inline

Below a certain size, describing a file costs more than sending it, the rule
of section 34.2, "send whichever is smaller", applied to content. `b:` carries
...
A larger inline file splits per section 7.6 with one added rule: **`b:` splits
...
is the section 7.6 procedure with the empty join: concatenate the `b:` values
...
anything larger travels as section 7.7 says. The two never mix, a file is
...
(section 9.7.1). Everything else still answers only what `q:` requested.
...
attacker never held (section 9.7.1).

---

## 9. Relaying and carried messages
...
### 9.2.2 When the sender names the relays
...
shared bearer where everybody hears everybody, section 6.2.1 leaves exactly one
...
**A station that is named may still decline.** Section 9.12.2 says it of
...
budget (section 30), and one that carries nothing is still a good citizen.
...
route taken, and every relay appends to it. `route:` (section 9.10) is the
...
**The section 6.1 budget still binds**, counted from `via:` exactly as before.
...
**Section 9.2.1's cancel does not apply to a named hop.** Under this section at
...
**Carrying is not relaying, and section 6.3 is not suspended.** A holder may
...
bearers it has, exactly as it would for any other packet. Section 12.0's rule
...
section 6.2 as before, so `relay:` narrows the traffic only among stations that
...
(section 7.6), and with the `via:` it accumulates a two-hop path adds about
...
Section 9.9 records that each `via:` which arrives is a route that actually
worked. `relay:` is how a station acts on that knowledge next time.

### 9.3 Carried messages
...
and they are different numbers (section 9.8).
...
A packet with `dest:` is **not** bound by the three-relay limit of section 6.1.
...
An `sos` is not marked `urgent`; it is an `sos`, and section 6.1 already gives
it nine relays. `urg:` is for ordinary traffic that happens to matter.

### 9.6 What a carrier can read
...
read it. This is the same rule as section 7.2 and needs no new mechanism.

A sealed body is longer than the text it replaces, so a long carried message is
split into parts (section 7.6). Every part repeats the routing keys, since a
...
15.1 ties decimal places to precision, and here that is a privacy control
...
and `r:` together (section 7.1). The result is evidence that the holder of
...
| `s:` | Means | Signed |
|---|---|---|
| `ack` | it reached a device | **by default** (section 9.7.1) |
| `read` | it was opened | **by default** (section 9.7.1) |
...
receipt travels home as section 9.7 describes, and a carried message is
...
person agreeing to anything, and that remains `s:sign` (section 9.7), which is
...
an attacker. `s:ack` is not merely a note to the sender: section 8 has every
...
must treat it the same way: unverifiable is not "probably fine". Section 9.7
...
answer is to send **fewer** receipts, section 9.7.2 already spends them only
...
section 22.1 counts a retry against the same budget as saying it the first time,
...
|---|---|
| the peer's beacon, heard recently | section 15.6, it already names the station and where to write to it |
...
what store-and-forward was for (section 6.3).
...
- **The bearer sets the cadence, and the strictest one binds** (section 22.1).
...
(section 8.1); adding `dest:` and `near:` moves which range. The same rules
...
### 9.9 The same message by two routes
...
never break. A receipt is idempotent (section 9.7): re-airing it asserts
...
### 9.10 Recording the route in the receipt
...
Because `sig:` covers everything except itself (section 7.1), the signature
...
(section 7.4) compose with scope like any other field.
...
**A `local` packet is not carried.** Section 9.4 exists to deliver somewhere
...
the rules in section 6.1 do not reach it, and this is why it has to be
said separately.
...
That holds for a closed group too (section 10). A member list decides what a
...
A packet that must not be read by strangers uses `x:` (section 7.2), and one
that must not travel uses both.

### 9.11.4 Against the other limits

`scope:` is an additional constraint and replaces nothing. A packet still stops
at the relay limit of section 6.1, still expires at `until:`, and still gets
...
This is the missing half of section 6.4. Carrying toward a place works when the
...
of the lookup it does for itself), because section 12.8.1 routes held mail by
...
failing that to any station advertising `serve:archive` (section 9.2).

### 9.12.2 Cancelling one

Plans change earlier than they were meant to. A declaration is withdrawn by
naming it, exactly as a warning is (section 11.2):
...
### 9.12.3 Asking whether there is mail
...
The answer is the `mail:` of section 15.6.5, narrowed: how many messages this
...
(section 33.2), which replays the mail itself, a page of packets, and a place
in the asker's hourly budget (section 22.2), to answer a question whose usual
...
12.11) and the same budget as everything else. Any asker may be answered,
...
An open group (section 7.3) has no door. Anyone may post to `LISBOA`, nobody may
...
callsign and a key (section 7.3):
...
(section 7.3), and `nick:` gives the group a readable name that is shown only
when the signature verifies (section 6.3.1). A group needs no announcement
...
administration, and section 15.6 already describes what handing a group key over
means.
...
### 10.3.1 Nobody is a member without saying so
...
The reason is section 15.7: the roster is public, permanent and
...
person; section 15.2 already says a listing "is a claim by the parent and
...
somebody who left by replaying an old offer, which is the harm section 10.3.1
...
Note what is **not** inherited from section 9.12.1: a mailbox declaration is
...
### 10.5 Expiry, and what a quiet group does

`until:` on a grant is optional, exactly as it is on a mailbox declaration
(section 9.12.1): a grant without one is open-ended and stays in force. A
...
Section 29.5 already refuses this inference for challenges, "treating silence
...
group; section 23 makes the same argument about encryption, and it applies here
...
None of this contradicts section 9.11.3. A group is still an address and not a
...
### 10.8 Bootstrap, and not becoming a weapon
...
Two limits keep that from being an amplifier, both following section 29.4:
...
of section 6.1 and does not earn a fresh three hops by being re-signed.

### 10.9 Propagation and discovery

Sections 10.1 to 10.8 say what a group is and who may act in it. They do not
...
#### 10.9.1 Hosting is a directory line

A station that keeps a group's traffic says so the way section 12.9 already
...
callsign is a callsign (section 15.1), so a directory line naming a group is
already legal and already means what it needs to mean. A beacon is the wrong
place twice over: section 22.1 asks that service announcements go out "on a
...
and section 30.3 already forbids saying so: "the claim is 'ask me', never 'I
...
#### 10.9.2 Depositing a group's traffic

Any member may hand a group's packets to an archiver, and to a always-on archiver
in particular. This is section 15.8's rebroadcast with a destination: the
...
- A deposit is **addressed, never broadcast** (10.8), because a deposit is
  work asked of one station rather than an announcement to the room.
- A deposit is **metered**. Section 30.2 is the rule, "serving a stranger is
  ...
  Section 30.1's "a retry is not a new packet" applies to the depositor: a
  deposit that went unanswered is re-sent against the same budget as the first.

#### 10.9.3 What an archiver keeps
...
Section 12.11 already orders eviction by class and then by age, over three
...
no `hold:` declaration governs it, so under 12.11 alone a group's history is
...
is a fourth axis on 12.11 rather than a change to the three it has.

This changes nothing a client may assume. Section 30.3 already says a client
...
### 11.1 The reply, immediately and again later
...
| `206` | part of the answer, more on request (section 11.2.1) |
...
### 11.2 The commands this document defines
...
differently, and it is specified in section 11.8 because what it costs,
...
never how it should travel; a station advertising `serve:files` (section 13)
...
64 kB and the 64 kB that arrived are verified against the piece list (7.7.2)
...
and `until:` bounds the stay, the discipline of section 12.7: a deposit is a
...
receipt for bytes**, section 9.7's receipt discipline applied to a file. A
...
A photograph left with the mailbox station a `t:mailbox` (section 9.12)
...
### 11.2.1 What comes back

The answer is the ordinary sequence of section 33.1, accepted, then done:
...
A station that keeps a spool says so with `serve:archive` (section 13). What it
keeps, for how long and for whom is its own to decide and to change: section
30.3 says why this document sets no retention period, and section 22.2 what a
station owes a stranger regardless.

### 11.2.2 A file transfer, on the wire
...
with a working-channel invitation (section 14.7) and the other agrees or
declines before a byte moves.

### 11.3 Keeping it out of the conversation
...
**Neither is replied to or reacted to** (section 7.5). They are protocol
...
Signing is the default everywhere (section 7.1); here it is a requirement, and
...
Five minutes rather than the sixty seconds section 29.4 gives a challenge,
...
**Commands are never carried.** Section 9.4 exists to deliver somewhere else
...
Section 11.9 is how the owner puts that list on the wire; checking it is still
...
rather than a hole in section 33.3. Everywhere else a bot reads `cmd:` and
...
Three rules on top of section 7.6, all of them about not acting too early.
...
### 11.6 Interpretation is not authorisation either
...
The allow-list of section 33.4 matters more here than anywhere in this document,
...
### 11.7 Devices: a shared vocabulary for acting on things

A command word is agreed between two stations (section 11), and between two
...
in the keys of this document, never in `arg:` (the rule of section 33.2):
...
counterpart of a measurement key, `temp:` (section 15.3) reports what IS,
...
`code:` table of section 33.1 needs nothing added:

```
151  t:result f:X1LAMP d:X1A67X ts:2026-08-19_18:30:05 r:1cc8af code:200 state:on level:40% sig:<60 characters>
```

**Reading a device is the existing request**, with one new word (section 8):
...
way every observation in section 15 already travels, a doorbell press is
...
**Section 11.4 is not optional here, and a lock is the reason it is written
...
The 300-second window and the derived-identifier idempotency of section 33.4
are what keep a recorded `state:unlocked` from opening the door tomorrow,
and carriers drop commands rather than park them (section 33.4), because
...
(section 15.5), and acted on by a person rather than a mechanism.

### 11.7.1 Controllers, and the X4 callsign
...
Commanding it is section 33.7 unchanged, because an `X4` station is a
station:
...
The allow-list of section 33.4 is kept **per device**, on the controller,
...
worth knowing before trusting its claims (section 15.5). It is
self-generated, so it never originates on licensed spectrum (section
6.4.1). And a controller that dies takes its devices off the air exactly as
...
so section 33.4 applies in full and without exception: unsigned or
...
(section 13) costs about ten bytes and makes a fleet's version spread
...
operator's, exactly as section 33.4 says authorisation is (the allow-list
itself is set on the wire by section 11.9). And it defends nothing against
...
### 11.9 Owning a station, and what the owner sets

Section 11.4 says a signature proves who sent a command and an allow-list
...
one its owner can repair, which is the position section 11.8 takes about
firmware, and the same one is taken here. Selling the box is handing over the
box: erase it, and the buyer claims it.

**`owner:` is the allow-list of section 33.4, and only an owner may change it.**
...
**What the owner sets, on `cmd:set`.** The command of section 33.7 already
...
| `serve` | `words` | what it does for others (section 13); `none` for nothing |
...
carried, because a result states what IS (section 33.7), and the four together
...
11.4 still leaves that one to the device.
...
anyone.** `serve:` is the station's announcement (section 13) made
...
which section 22.2 says is still a good citizen.
...
3. by `urg:`, `urgent` before `high` before `normal` before `low`, with a
   stranger's packet counted no higher than `high` (section 9.5, and the
...
it, because section 9.5 already says what a stranger's `urg:urgent` is worth:
...
`q:policy` and `s:policy` are the words of section 8 doing what they always
do, and the answer is an observation because that is what a state reported
unasked-for or asked-for has been since section 33.7.

**A policy survives a reboot, and an old one cannot be replayed.** Section
11.4 gives a command 300 seconds and a station's memory of one 600, which is
...
station that says `use:all` still meters strangers under section 22.2 and may
...
A station that was on a network keeps it until the new one has joined. After
the failure it goes back to the network it had, and only then answers, from
there: the `500` then reads `wifi:up ip:<the old address>` beside the reason
the new one failed. A station reached over the very network being changed
would otherwise air its `500` on a link it no longer had, and a typo in a
password would have cost the owner the station.
...
Who keeps what and who does what for the network: the archiver, the service words a station advertises, channels, and the observations stations air.

## 12. Archiver

*Publishing, and the archivers you choose.*

An **archiver** keeps XPRS packets and answers for them. It holds a spool of
what it heard and re-airs it on `cmd:history`, keeps the publications of the
stations that chose it, holds mail for stations that are absent, and publishes a
directory of who-keeps-what. The goal: **a message, a post or a file survives
its author going offline, and an absent station stays reachable.**

**Every device is an archiver; scale is a setting, not a kind.** There is no
server class. A pocket phone archives its operator's own words and those of the
callsigns they follow, and announces nothing. A powered station archives
everything it hears, carries mail, bridges bearers, and announces
`serve:archive`. An always-on internet machine does the same at larger budgets.
Archivers differ in what they answer, never in what they are, and a reader
learns which is which by asking, so the format needs no word to rank them.

**The operator sets the rules, like a NOSTR relay operator.** The owner decides
what it keeps and for whom: their own traffic, the callsigns they follow,
stations heard nearby, or anyone, under a quota. Silence is not consent: a
...
station that did not pick it. A station naming none MAY let one be
auto-selected, a volunteer advertising `serve:archive` that accepts it; naming
one explicitly overrides that. Zero archivers is a valid, private configuration:
such a station serves its own publications to whoever asks over the radio, and
is simply not findable by someone who was not listening.

*(Synchronisation BETWEEN archivers is out of scope here and specified
separately.)*

### 12.0 One role, and it does not change with the bearer

`serve:archive` is the only word, and covers everything: keeping packets and
answering for them (sections 12.1 to 12.6), holding mail (section 12.7) and
saying how much (sections 15.6.5 and 9.12.3), delivering it when the recipient
turns up (sections 12.8.1 and 12.8.2), and publishing the directory (section
12.9). There is no separate word for the pointer half or the storage half; a
pointer-only station and a keep-everything station announce the same word and
differ only in what they answer. An operator's interface MAY present the offers
separately, and should, since holding somebody's files and remembering where
their files are cost very different things and either may be granted alone. On
the air it is one word.

**The role is identical on every bearer.** A `cmd:history` over Bluetooth, LAN,
ESP-NOW, LoRa or the internet gets the same records for the same window, the
same `code:202` / `code:200` / `code:206` / `code:404` / `code:429`, the same
paging, budgets (section 22.2) and signatures. A bearer decides how bytes
travel, never what a station will do; an archiver that behaves differently on
one is broken, not slower. This is written down because the failure is
invisible: a station that answers on one bearer only still beacons, still
archives, still asks its own questions correctly, logs no error, and every asker
on another bearer just hears silence.

**An offer covers every bearer, not the one it was heard on.** `serve:archive`
offers to archive on ALL of a station's bearers. A reader that hears the offer
over the LAN but can reach the station only over LoRa still asks over LoRa, and
is owed the same answer.

**The one place a bearer legitimately decides anything** is choosing among
several paths to the SAME station. An archiver with more than one path to its
asker picks the highest usable bandwidth among those it has recent evidence are
working; reliability outranks raw speed, because a fast path that has carried
nothing lately is a guess and a slower one that answered a minute ago is
knowledge. The bearer an ask ARRIVED on is therefore a sound default: it just
carried a packet from the station now waiting, the freshest evidence of a
working path. But it is a default, not a constraint. If the archiver knows a
better path it uses it, and if the arrival bearer cannot carry the answer it
does not give up.

**Where a station cannot tell which path reaches the asker** (the broadcast
case), it answers on every bearer it can transmit on. Duplicates deduplicate on
the section 5 identifier, so the reader sees one copy however many arrived. On
an expensive bearer an archiver MAY hold back a page it already sent somewhere
faster; it MUST NOT hold back a `code:404` or `code:429`, because a control
packet that does not arrive is indistinguishable from a station that is not
there.

**A claim must not silence proof.** A `link:` (section 15.6.1) is a station's
own word that it stands on a radio; it is a claim, not a measurement that you
share that radio. An internet path carries no `link:`, because it is not a radio
a station stands on, and its only evidence is a live route the transport holds.
Until `via:` distinguishes a beacon heard on the air from one relayed home
(section 37, not yet implemented), a station on another network advertises
`link:ble` into your ear as though in the room, so a path merely claimed MUST
NOT suppress one you have confirmed: when both stand and nothing confirms the
claim, answer on both.

None of this is `scope:local` (section 9.11.1), which is the author's property
saying how far a packet may travel, and it constrains the answer like anything
else. What this section forbids is a station narrowing its OWN service to one
bearer of its own accord.

### 12.1 What a publication is

An archiver holds two things, and the difference is `d:`.

| | What it is | What the archiver does with it |
|---|---|---|
| **A publication**, no `d:`, a type below | offered to whoever is interested | answers queries about it, to anybody |
| **Mail**, anything carrying `d:` | addressed to one station | holds it for that station and tells them; never offers it to a third party (section 12.7) |
...
The last two make a gateway useful beyond its own hill. A gateway's own
`t:observation` is a reachability record: `f:` says which gateway, `hears:` the
radio-only stations at its ear now, `ts:` how fresh the claim is. And a gateway
passes on the `t:identity` and `t:mailbox` packets it hears verbatim, which
section 34.2 makes safe: the author's signature travels with the packet, so a
copy proves itself against the author's key, not the gateway's honesty.

Neither published nor held: `ping` and `pong`, which measure whether a path is
alive right now.

**Addressing decides, not the type.** The packet already says who it is for, so
a `t:message` is mail, a `t:command` to a sleeping station is mail, and a
`t:warning` with no `d:` is a publication.

### 12.2 The archiver is sent the packet, not a description of it
...
150 bytes, the section 24 example, unchanged. There is no envelope, no summary
record and no transformation step, hence no second schema to keep in step with
the first and get subtly wrong.

**Why this is affordable.** The file layer stores pointers and never content
because a file is megabytes and a pointer 176 bytes. A publication is at most
250 bytes: pointer and content are the same order of size, so the rule is not
"always send pointers" but "send whichever is smaller", and here that is the
packet. The inline file lane (section 7.7.4) applies the same rule: under ~900
bytes the content IS the smaller thing, and it rides the packets.

**Everything a query needs is already in it:** `t:` the type, `f:` the author,
`ts:` when, `pos:` where, `dest:` and `near:` the region, `until:` expiry,
`scope:` reach. And the signature travels with it, so an archiver passing on a
third party's publication can neither forge, retarget nor resurrect it, and the
receiver verifies against the author's key (section 7.1). That is what makes
gossip between archivers safe.

### 12.3 You choose your archivers

**A station pushes to the archivers its operator picked, and to no others.**

- The list is configuration: editable, with defaults, and adding or removing one
  is an ordinary act rather than a reinstall.
- **Zero archivers is a valid, working configuration** (section 34.5): the
  station keeps its publications and serves them to anyone who asks over the
  radio, and is not findable by someone who was not listening. A station that
  talks only to those in range is private, not degraded.
- The choice is **per station, not per operator**: one phone may push to two
  archivers while its owner's shed node pushes to none.
- An archiver may decline what it is offered (section 22.2).

The station remembers **what each archiver has already had** as a position in
its own log rather than a time (a position cannot skew, and a clockless device
can still persist one). A reconnect resumes; it does not re-send.
...
thing (section 22.1). So the push belongs to a bearer that is cheap and
plentiful, internet or a wired link, and the radio carries publications the way
it always did: as packets, to whoever is listening, once.

### 12.5 Asking a station directly
...
type, region and radius, or time window. Each reads a field the packet already
carries, so the query surface needs no vocabulary of its own and cannot drift
from the format it queries.

One reading rule makes the reachability question askable without any new word:
**`only:` matches a callsign wherever the packet carries it**, as author, as
addressee, or inside a list field (`hears:`, `hold:`, `via:`, `grant:`). It is a
callsign and never a type; section 33.2's `kind:` names a type, and the two
combine rather than compete. So "everything about X1BOA3" includes the gateway
observations that list it as heard:
...
The reply is the section 11.2.1 replay, `code:202`, then the original packets,
...
X1BOA3 was at X3RLY7's ear forty seconds before the ask. The reader now knows
which internet-connected station is one radio hop from the recipient, and how
stale that is (`ts:` is the freshness, and a reader that gets three gateways back
prefers the newest). A miss is not a dead end: an archiver that does not archive
the callsign answers `code:404` with `m:try` naming peers whose directories list
it (section 12.9).

What matters is not the syntax but that the reader chose the archiver, the
publisher chose the archiver, and neither choice had to be the same for the
network to work.

### 12.7 An archiver is also a mailbox

**The sender is usually gone**: a message written on a phone that then pockets
its radio. If delivery depended on that phone still being reachable when the
recipient next wakes, most messages between people not simultaneously awake
would never arrive. Store-and-forward exists for this (section 6.3), and an
archiver is its best carrier: always on, addressable, chosen deliberately. So
**mail is handed to an archiver too**, which tells the recipient something
waits, and an archiver is a natural entry in a station's `hold:` list (section
9.12), which already exists and needs nothing added.

**Privacy is the content's problem, and the format already solved it.** Seal the
body with `x:` (section 7.2) and the archiver stores something it cannot read:

```
t:message f:X1QZ3N d:X1RD89 ts:2026-08-13_10:14:00 x:pQ4m9xT2vB8kR until:2026-08-20_00:00:00 sig:<60 characters>
```

157 bytes. `t:`, `f:`, `d:` and `ts:` stay in cleartext, because a station that
cannot see who a packet is for cannot deliver it. So the archiver learns that
X1QZ3N wrote to X1RD89 at that minute and how often the two do that, exactly as
every APRS-IS server did for every message. Encryption protects the contents;
choosing your archiver is what protects the pattern.

**It is a hold, not an archive.** `until:` bounds it, and the archiver releases
its copy the moment it hears a receipt whose signature it has verified (section
8; section 9.7.1 insists on the signature so a stranger cannot delete other
people's undelivered mail). An archiver may refuse to carry mail at all, or
carry it only for stations it knows (section 22.2).

### 12.8 The gateway is the last mile

Sections 12.1 and 12.6 built the outbound half: the gateway told the archiver
who it hears, and a sender found the gateway. This is the return leg, how mail
deposited at an archiver reaches a station that never touches the internet. Mail
is never offered to a third party (section 12.7), and a gateway asking for
somebody else's mail IS a third party; section 9.12 solves it only for stations
that declared a mailbox, which a solar tracker on a ridge cannot do.

The resolution splits on what the mail is:

- **Sealed mail travels on the strength of the seal.** An `x:` body (section
  7.2) is ciphertext to everyone but the recipient; carrying it is what custody
  already is (section 9.6), and one more carrier discloses nothing the airwaves
  would not. An archiver releases sealed mail to a station whose own published
  observation currently lists the recipient in `hears:`, the gateway one hop
  from delivering. A false `hears:` buys only ciphertext and the envelope
  metadata the archiver already held.
- **Clear mail is released only to a declared holder** (`hold:`, section 9.12)
  or fetched by the recipient itself. Disclosure follows the recipient's stated
  arrangements or nobody's.

Delivery is then ordinary. The sender seals and deposits:

```
162  t:message f:X1QZ3N d:X1BOA3 ts:2026-08-17_14:02:00 x:pQ4m9xT2vB8kRZ7cW0yLuJ3gRhN8sEiDoQ6vXaB1MnYw sig:<60 characters>
```

The gateway X3RLY7, whose observation listed X1BOA3, collects it and airs it on
the radio under the custody rules of section 9, and the receipt comes back the
same path:
...
The recipient signs the receipt, so the archiver verifies it and releases its
held copy (section 12.7), the gateway archives its own, and the sender three
networks away knows the tracker on the ridge has the message. The gateway was
trusted with nothing but ciphertext and effort.

### 12.8.1 The return leg, automated

Section 12.8 built the last mile by hand: someone had to ask. Two behaviours
remove the hand, one MUST and one MAY, and together they make a deposited
message ARRIVE rather than wait to be collected.

**A station holding mail for X delivers it the moment it hears X** directly
(section 15.6.3's sense, no `via:`), on the bearer X was heard on, the freshest
evidence of a working path (section 12.0). This is not a poll: the trigger is
the packet from X itself, not a ten-second check that spends battery on a
question the air already answered. Retries of an unacknowledged copy back off
(the same packet re-aired), which is NOT the same as handing over the NEXT ones
(section 12.8.2, which should be prompt). A verified `t:receipt` (section 9.7.1)
ends the retries and releases the held copy, and every OTHER holder that hears
the receipt releases too, so a chain of custodians drains instead of delivering
twice. Airtime spent on delivery is metered under section 30.

**A station holding mail for X MAY hand it toward where X actually is,** in
order:

1. X's own declaration (section 9.12): the recipient's word beats every
   observation. This is the first time `hold:`'s preference order is consumed
   rather than merely stored, and it is consulted first.
2. The gateway whose gossip entry for X is freshest (section 12.9.4): a station
   that heard X five minutes ago on LoRa is one hop from delivering, whatever
   continent this holder is on.

The hand-off is CUSTODY (section 6.3), not content sync: the author's packet
travels byte for byte with its signature, `via:` gains the holder's callsign,
the section 6.1 budget and 6.2 loop check apply, and section 12.8's disclosure
rule stands. **Once per holder**: a packet is forwarded one time, and one that
comes back (per `via:`) is not forwarded again. This migrates the mail toward the
recipient's radio horizon while both parties sleep, the whole difference between
a mailbox and a pile.

### 12.8.2 Draining a backlog, and what a holder must remember

A returning station has more than one message waiting, and both obvious readings
of "delivers it" are wrong: airing everything at once spends the channel the
station is trying to use, and airing one per sighting takes as many sightings as
messages.

- **A release is a page.** The holder hands over the newest few and the rest
  wait for the next sighting, usually seconds away, because a station that just
  spoke is about to speak again. The page size is the holder's budget (section
  30); what matters is that it REMEMBERS where the page stopped, or it delivers
  the same few for ever and the recipient never sees the fifth.
- **A short page ends the round.** Reaching the end of what it has, the holder
  forgets the mark and the next sighting starts at the newest again, which is
  also how mail that arrived while the backlog drained gets its turn.
- **Two different waits, and confusing them is the bug.** Backoff applies to an
  UNACKNOWLEDGED delivery: the same packet, re-aired, less often each time.
  Continuing a backlog is the NEXT packets, to a station demonstrably present,
  and should be prompt. A holder that applies its retry backoff to the backlog
...
- **An acknowledgement is durable or it is worthless.** A receipt releases the
  copy (section 9.7.1); a holder that forgets the release when its power blinks
  re-airs mail the recipient already took, at every sighting after, because the
  archive survived the reboot and the memory of the receipt did not. A holder
  that keeps mail across a restart MUST keep the receipts across it too.
- **The recipient may ask instead of waiting.** `q:mail` (section 9.12.3) says
...
federation of archives from becoming one pool of spam. A peer's archive is that
peer's admission decisions, and bulk-importing it imports every decision the
other operator got wrong, at zero cost to whoever made them. Content enters an
archiver exactly one way (section 34.3): from the callsigns its operator chose
or agreed to receive. (The gateway pass-through of section 34.1 is the same
rule: the gateway is a depositor this archiver accepted.)

What archivers DO exchange is a **directory**: which callsigns each is archiving
or receiving from, and when it last heard each. A text listing in the section
7.7.2 family:
...
One line per callsign, `call` then `ts`, sorted by callsign. The directory is an
ordinary content-addressed file: named in its archiver's signed announcement,
...
`count:` stays what section 13.0.1 made it, records held, never callsigns; how
many callsigns the directory lists is learned by fetching it. The economics are
section 34.2's: a line costs about 28 bytes, ten thousand callsigns about 280
kB, and an unchanged directory has an unchanged hash, so polling a quiet peer
moves nothing. A consumer stores pointers (callsign, which archiver, how fresh),
never the content behind them. A false directory line is priced like a false
`hears:` (section 15.6.3): one wasted redirect per reader, because the content
still answers or fails on its own signatures.

**Discovery needs nothing new.** A station finds an archiver four ways:
`serve:archive` heard on the air; another archiver's copy of that same signed
announcement (section 34.2 makes verbatim relay safe); `cmd:history kind:service`
against any known archiver, which replays every `t:service` it holds; and the
redirect that answers a miss:
...
names, in `m:try`, the peers whose directories list it (the section 11.2.1
alternates for `429`, extended to the miss). The reader asks the named peer
directly; the first never proxies, because proxying is how content would cross
the line this section drew. The result is a federation of small archives, each
vouching only for what its operator chose to keep, joined by directories that
say who keeps what.

### 12.9.1 What crosses between archivers

Everything archivers exchange is a **signed original**, verifiable against its
author's key rather than the peer that handed it over (section 34.2). Four
things cross:

| What crosses | The question it answers |
|---|---|
| a `t:service` announcement, verbatim | who offers what; how an archiver is found at all |
| the directory file (section 12.9) | which callsigns deposit where, and when each was last heard |
| the archiver's own `t:observation`, with `hears:` | which stations are at its ear now, and how fresh that claim is |
| `t:mailbox hold:` declarations, verbatim | which stations chose which archiver to hold their mail |

The last three ARE the gossip of section 12.9.4, reachability carried by packets
that already existed. Each is signed by its author, so a relaying peer can
neither forge nor retarget it, and a reader weighs its `ts:` for itself.

**What does not cross is the store.** Archivers do not synchronise message
archives; the directory exists precisely so they need not. The one temporal fact
that flows is the last-heard timestamp riding each directory line. And mail never
crosses at all: a held message leaves an archiver toward its recipient, a
declared holder, or a `hears:` gateway (sections 12.7, 12.8), never toward a peer
archiver.

### 12.9.2 The archiver is the seeder

Publications carry `file:` references (section 7.7); the bytes behind a reference
live wherever somebody kept them. For those bytes the archiver plays a torrent
seeder's part: it holds a full verified copy, says so when asked, and serves it
without involving the author.

The mechanics exist and this adds none: `q:have` (section 8.1) asks whether the
bytes are held; `cmd:file` fetches them, `off:` resumes a dead transfer;
`cmd:put` deposits them under a stated `size:` and `until:`; an `ih:` reference
(section 7.7.5) lets the same copy seed a BitTorrent swarm. A station doing this
announces `serve:files` beside `serve:archive`, since a pure file host with no
spool is a real station, but on an archiver the two halves reinforce: the spool
holds the announcement that names the file, the file store holds the bytes it
...
Most of what a community publishes is about a PLACE (`info`, `warning`, `event`,
`status`, `blog`, and a town channel's undirected `message` traffic). A
neighbourhood whose only archiver went dark loses its noticeboard, so two
archivers placed near each other MAY keep each other's community publications
alive, within bounds:

- **Nearby is declared, then measured.** Both carry `pos:`; each computes the
  distance. The reference threshold is 25 km, tuned to the terrain; the point is
  a shared audience, not a number.
- **Opt-in.** Nothing obliges an archiver to mirror its neighbour.
...
This is section 12.10's meeting with a `kind:` filter, on section 12.10.1's
per-peer watermark, under section 22.2's budgets, a standing catch-up rather than
new protocol. The peer re-airs ORIGINALS, each verifying on its author's
signature, entering the asker's store through its own admission; section 12.9's
line is not crossed, because nothing is imported on the peer's authority.
Replayed packets deduplicate on the section 5 identifier and expire on their own
`until:`, so the two stores converge on the union of both neighbourhoods'
publications for those kinds and nothing else. Mail takes no part: a `d:` packet
is section 12.7's business however it travels.

### 12.9.4 Gossip: who was heard where

The exchanges above have a collective name: **gossip**. Archivers never sync
content, that rule opened this section and stands. What they gossip is
REACHABILITY: which callsign was heard, by which station, on which bearer, when.
Each archiver keeps its own bounded table, built from what it overheard and what
peers passed on, and the network's answer to "where can X be reached" is the sum
of many small tables rather than one central one. That is the load-bearing
difference from APRS-IS, whose central servers hold the only copy of exactly
this.

Gossip rides existing packets, a signed `t:observation` with `hears:` (15.6.3), a
signed `t:mailbox` (9.12), a `t:service` announcement (24), relayed verbatim
under section 34.1. There is no gossip packet type, because a new envelope for
old facts would be a second schema to drift.

**The three layers**, consulted in this order:

| layer | holds | fed by | expires |
|---|---|---|---|
| **L1, declared** | the callsign's own `hold:` list, in its order | its own signed `t:mailbox` | its own `until:` |
| **L2, visit history** | the last K distinct archivers that heard the callsign DIRECTLY on a short-range bearer; per entry: archiver, first heard, last heard | radio truth only (below) | **never**, the ring evicts the oldest distinct archiver. K is 100 for the reference station classes |
| **L3, live sightings** | the freshest (gateway, bearer, time) claims, at most G per callsign (reference G = 8) | any verified sighting, internet included | a TTL (reference 24 hours) |

L1 is the recipient's word and beats everything; L3 answers "deliver NOW"; L2
answers "where does this callsign tend to surface", the marina a boat checks in
at, the repeater a commuter passes, the knowledge that routes a message
deposited months after the last sighting. L2 is the one layer allowed to live
forever, because it is small (K entries per callsign), slow-moving and
irreplaceable.

**Validity: gossip is a priced claims market.** Every entry is credited to the
station that SIGNED the packet it came from; unsigned observations feed nothing
beyond the local air view, and forged ones are dropped by the existing
verification. Three walls:

- **L2 admits radio truth only**: this archiver's own direct hearing, or a
  verified observation whose `link:` names a short-range bearer. No packet that
  travelled only the internet writes the durable layer, so poisoning the visit
  map requires transmitting on the air somewhere, with a signed identity, inside
  radio range of a station that keeps records.
- **Per-signer quota**: one observer's gossip is accepted at the rate its own
  adverts arrive (section 30's metering applied to ingestion).
- **Byte budget with eviction**: the table lives under a class-sized cap, and
  when full, L3 evicts stalest-first before L2 evicts anything.

What spam buys after all three walls is what a false `hears:` always bought
(section 15.6.3): one wasted redirect per reader, never a delivery.

**The arithmetic.** An entry packs into about 20 bytes; a full K=100 visit ring
is ~2 KB per callsign; a pocket or desktop node's reference cap is 5 MB, room for
the visit history of ~2,500 callsigns, which is a town. Stations below that class
do not try; stations above it raise the cap.

**Need-to-know, not replication.** A station keeps gossip in proportion to its
duties. An ESP32 archiver keeps the L1 declarations that name it, its own
direct-heard rings, and L2/L3 rows only for callsigns it is currently holding
mail for, a few kilobytes; everything else it resolves when a duty arrives, by
asking a bigger archiver: `cmd:history only:X1BOA3 kind:observation` (section
12.6) replays the signed sightings, which the asker verifies and caches into its
L3. Bulk gossip is the history replay; it needs no new verb. (The XDIR census
file is the bulk form for a station that can serve files; one that cannot omits
the `file:` reference from its announcement and the ask-form carries the load.)

**What an always-on archiver must be.** Scale is a setting (section 12.0), not a
class, and `serve:archive` is the only word. But the archivers the network
actually leans on share qualities a pocket device cannot promise, and a station
should not invite asks it cannot serve (section 22.2):

| the quality | what it means in practice |
|---|---|
| **addressable** | reachable by a DIRECTED packet, not only broadcast; an archiver that cannot be asked cannot be leaned on (12.12.1) |
| **deep** | a spool measured in RECORDS, not in leftover flash; a store holding only a busy day is a pocket archiver, honest as one |
| **budgeted for it** | section 22.2's reference numbers raised as far as the machine allows, because six replays an hour is a pocket budget |
| **concurrent** | more than one ask in flight, or the second asker of any minute is refused |
| **complete** | gossip kept for every callsign it learns of, without the need-to-know cap a small station applies |
| **awake** | always on, so it heard what everyone else missed while they were asleep |

None of this is a separate role: a station that can do some of these and not
others is an ordinary archiver doing well, and `serve:archive` says so honestly.
On the internet, where stations whose neighbours are all remote are invisible to
each other until one archiver they can all address holds the conversation
(section 12.12.2), an always-on archiver is where public traffic is pushed and
pulled. The same role runs on a satellite or beyond Earth; every hop is
store-and-forward, which does not care how long the light takes. Humble stations
ask such an archiver what their own gossip does not know, the way section 9.12.1
already falls back to `serve:archive`.

### 12.10 Two archivers meet

A station that was away has a hole in its archive as wide as its absence, and the
tool that fills it exists: `cmd:history` (section 33.2). This section adds only
WHEN to use it on the archiver's own behalf.

**On sighting**, an archiver that hears `serve:archive` from a station back in
radio range, or just powered on, asks for the window it missed:

```
t:command f:X3ARC1 d:X3ARC2 ts:2026-08-20_08:30:00 cmd:history since:2026-08-20_06:12:44 sig:<60 characters>
```

`since:` is the newest packet the asker held WHEN THE HOLE BEGAN (at power-on,
the newest already on disk), not the live newest, which fresh traffic has already
pushed past the missed window. Overlap is harmless: replayed packets deduplicate
on the section 5 identifier. The peer re-airs its spool for that window under its
own section 30 budget, usually on the bearer the ask arrived on (freshest
evidence of a path, section 12.0) but never merely because the code reached for
it. The replayed packets are ORIGINALS, each verifying on its own signature;
nothing is imported, and both archives converge on the union of what either
heard.

Discipline: ask once per peer per absence (a cooldown of at least ten minutes);
never ask without a synced clock (a `since:` from a wrong clock asks for the
wrong window); and a station with nothing missing asks for nothing, an empty
reply costs the peer its budget all the same.

### 12.10.1 The pocket device polls

Section 12.10 is for stations that notice an absence. A pocket device cannot: it
sleeps in a pocket, wakes on a desk, and has no idea which was an absence worth
naming. So it keeps a WATERMARK and polls.

The watermark is a persisted timestamp PER ARCHIVER: the end of the last window
that archiver answered for. Not one shared mark, since an archiver holding
nothing answers `code:404`, which advances its mark, so a shared one would let an
empty peer narrow the window on a full one standing beside it. The poll is the
same ask as any meeting:
...
bearers, where a locally-reachable archiver is. The cycle:

1. Every poll period (default TEN MINUTES), the device looks at the stations it
   heard DIRECTLY (no `via:`) within the beacon-staleness window and picks those
   announcing `serve:archive`.
2. To each, at most one ask per period: `cmd:history since:` the watermark. A
   fresh install has no watermark, asks for nothing, sets it to now and starts
   keeping.
3. THAT archiver's watermark advances to the ask's `ts:` on `code:200` or
   `code:404` (nothing held is an answer). It does NOT advance on `code:429`, on
   silence, or on `code:206`, which closes a page rather than the window (section
   11.2.1).
3a. An archiver's `count:` (section 13.0.1) MAY bring the next ask forward (the
   number moved), but MUST NOT postpone one indefinitely: the period in step 1 is
   a backstop always armed, because a `count:` can be absent or stale, and a
   device that lets a frozen number silence its poller stops fetching while
   looking healthy.
4. A single ask is bounded to seven days; a device away longer asks for the last
   week, and anything older is fetched deliberately, not by a background poll.

Ten minutes is not arbitrary: section 30's reference budget answers a known
caller six times an hour. A faster device steals its own budget; a slower one
merely learns the news later. Section 12.10.2 is what a station does with this
once it can see which of its archivers are busy and which have been silent since
spring.

### 12.10.2 The poll adapts to what it finds

One clock for every archiver is the right default and the wrong steady state: a
room silent since spring costs the same metered replay as a live one, and the
archiver pays for both. A station SHOULD keep a SEPARATE interval per archiver
and move it by what that archiver answers, measured free from the answers already
coming back:

| the archiver answered | the interval |
|---|---|
| a page with records we did not have | halve it, it is talking |
| `code:206`, more held than served | ask the continuation IMMEDIATELY |
| `code:200` or `code:404` with nothing new | double it |
| `code:429` | double it, and never faster than once a minute afterwards |
| silence | leave it; silence is evidence about the path, not the room |

**The ceiling is how long that archiver has been quiet**, counted from the last
time it gave this station something new:

| silent for | asked at most every |
|---|---|
| less than a week | 10 minutes (12.10.1's period) |
...
Section 30.2's reference budget answers a known caller six times an hour, which
is why 12.10.1's period is ten minutes:

- An ordinary archiver is never asked faster than that period. A station that
  polls faster does not get more news; it spends that archiver's cross-caller
  allowance and starves its own neighbours.
- An **always-on archiver** (section 12.9.4) may be asked far faster, reference
  floor **15 seconds**, not for budget reasons but because one replay runs at a
  time and a page takes on the order of fifteen seconds to air, so a faster ask
  arrives while the previous answer is still sending and is refused. A station
  MUST NOT have more than one unanswered ask outstanding to the same archiver.
- A station's own other devices (section 3.1) are not metered by the responder
  and may be asked at the same fast floor.

**`code:429` is the archiver's authority over the caller's cadence**, the one
answer that must always slow a station down. Treating it as silence, asking again
on the same schedule, being refused again, is a loop that never advances a
watermark and looks from outside like a quiet network.

**Nobody awake, nobody polled fast.** A device whose screen is off falls back to
the quiet ladder whatever the room is doing; a fast tier exists to put words on a
screen somebody is looking at, and running it into a pocket spends battery and
somebody else's budget to no end.

**Spread the herd.** A station SHOULD jitter each interval by a small fraction (a
tenth is enough), or many devices on the same nominal period arrive together and
the load an archiver feels is the peak, not the average.

Two properties make all of this safe to get wrong: a replayed packet deduplicates
on its section 5 identifier, so an interval too short costs airtime and never
correctness; and the watermark only advances on an answer, so one too long delays
news and never loses it.

### 12.11 When the store is full

An archiver's capacity is whatever its operator gave it, and when it is full
something must go. Section 30.3 stands: retention belongs to the station. What
follows is the DEFAULT the reference stations ship, because a default chosen
badly deletes somebody's mail to keep a stranger's chatter. Decided by class
first and age second, **within a class the oldest goes first**, and no packet
outlives its own `until:`.

Discarded first, kept longest last:

1. **The spool**, ordinary heard traffic: the bulkiest class and the cheapest to
   lose, since any peer's spool overlaps it and section 12.10 refills holes.
2. **Custody mail**, store-and-forward held for stations that did NOT name this
   archiver (section 6.3): losing it costs a delivery somebody else may still
   make.
3. **Declared mail**, for callsigns whose `t:mailbox hold:` names THIS archiver
   (section 9.12): those recipients chose this station deliberately and check it
   first, so it is the last thing an archiver may drop, and inside its `until:`
   it should never be.

The classes read straight off the packets: `d:` plus a `hold:` naming this
station is class 3, `d:` without one is class 2, everything else class 1. An
archiver that cannot take new mail without touching class 3 refuses out loud
...
from anywhere, reaches the callsign it names, wherever that callsign last touched
a radio.** In the order a message meets them:

1. **Deposit.** A sender (a phone on another continent, a server script, a
   station three bearers away) hands `t:message d:X1BOA3` to any archiver it can
   reach (sections 12.3, 12.7). The archiver holds it: already mail, already
   bounded by `until:`, already released by receipt.
2. **Consult.** The holder consults its gossip for X1BOA3 (section 12.9.4): the
   recipient's declaration first, the freshest live sighting second, the visit
   history third. A holder with nothing asks an always-on archiver.
3. **Forward.** The held wire moves as custody toward the named gateway or
   declared mailbox (section 12.8.1), signed by its author, `via:` growing,
   loop-checked, once per holder. It can cross the internet on one hop and a LoRa
   hill on the next; every hop stores, so nothing is lost to a link down today.
4. **Deliver.** The gateway that actually hears X1BOA3 releases the mail the
   moment it does (section 12.8.1), and the receipt (section 9.7) walks back
   through every holder and clears them all.

Against APRS-IS: an igate here is any archiver with a radio, and unlike an igate
it holds mail rather than forwarding into the void; the central servers are
replaced by gossip among archivers, with always-on archivers as optional deep
memory but no single table anyone must trust or fund. Last-heard routing is
APRS-IS's proven idea done with signed claims instead of trusted servers. And
absence, which loses an APRS message, merely delays an XPRS one, since every hop
stores, at the scale of a pocket or of the solar system.

What this does NOT promise: delivery to a callsign nobody ever heard (a
declaration can pre-position its mailbox), secrecy of the envelope (section
12.7), or that any station carry anything against its budgets (section 30.3). The
promise is narrower: if the callsign touches the network anywhere its gossip
reaches, the mail finds it.

### 12.12.1 Constrained internet transports

The chain crosses the internet on shared transports run by strangers, with their
own budgets. Operating experience is blunt: **a shared transport filters, and the
subset that reliably crosses is small, identity announcements and directed
messages.** Broadcast wapp data, bulk announces, anything addressed to nobody may
be dropped at any hop, silently. A design that works only on an unfiltered
transport does not work.

**The two guaranteed lanes**: (1) an identity announcement, the transport's own
currency, since without paths nothing routes; (2) a message directed at a named
destination the transport has a path to. Everything an archiver needs to say
across the internet MUST be expressible as one of these, and every rule below is
that applied to one exchange:

- **Asks are directed messages.** A station resolving a callsign across the
  internet (the section 12.9.4 miss) sends `cmd:history kind:identity,observation
  only:X1BOA3` with `d:` naming a chosen always-on archiver, not a broadcast
  question a filtered hop drops. `kind:` lists `identity` first deliberately: the
  observations that answer verify against keys the asker may never have met, so
  the identities carrying those keys must land first.
- **Replies are directed messages too.** A station answering an ask that arrived
  over the internet MUST send every replayed record and `t:result` on the
  directed lane to the asker (section 12.10's radio pacing stands for the local
  copy). A replayed record carries its AUTHOR's addressing, which on the announce
  lane is exactly the broadcast a filtered hop drops. The double lane costs
  nothing: both copies carry the same section 5 identifier and deduplicate.
- **The keys ride the same page.** A page of sightings about X is signed by the
  stations that SAW X. The responder prepends the newest `t:identity` it holds
  for each observer, once each, before the observations they vouch for, since a
  page of claims the asker cannot verify is a page it must discard, and asking
  the internet for each key separately is the round trip this avoids.
- **`only:` matches the subject, not just the correspondents.** For observation
  records, `only:X` matches a sighting OF X (an observation whose `hears:` lists
  X), not merely packets X sent or received, which would make "the sightings
  about X" an empty set by construction. (For every other record type, `only:`
  keeps its section 12.6 meaning.)
- **No gateway resolved is not a dead end: deposit.** A holder whose gossip names
  no gateway for X MUST NOT broadcast the mail into a filtered transport. It
  fires the miss-path ask and MAY deposit the held wire as custody with an
  always-on archiver (section 12.12 step 1, one hop up), whose gossip is the
  deepest available; a deposit is a directed message that crosses, `via:` grows,
  the loop check applies.
- **A sender is its own first holder.** A station sending `t:message d:X` parks
  its own signed wire for custody at the moment of sending, exactly as it would a
  stranger's: an airing is an ATTEMPT, and the obligation ends at a verified
  receipt (section 9.7.1). This is what starts the chain.
- **An always-on archiver keeps the chatter.** Signed observations are the wires
  a bulk-gossip replay serves; an always-on archiver that discards presence
  records answers every `kind:observation` ask with an empty page, since gossip
  stores conclusions and a replay may only re-air originals (sections 12.1,
  12.2). `serve:archive` therefore implies retaining observation and identity
  wires under the raised budgets of section 12.9.4 on every lane, internet
...
instruction beginning "forward port" has misread this document. What is NOT
constrained: a transport one operator controls end to end (a private hub, a LAN,
a point-to-point link) carries whatever its operator pleases, and the broadcast
forms elsewhere remain correct there.

### 12.12.2 Public traffic across the internet: the archiver is the meeting place

Section 12.12 routes mail to ONE callsign. This is the other half: **public
traffic, the broadcast addressed to everybody, does not cross the internet by
being broadcast.** It crosses because archivers hold it and others ask them for
it.

The failure is quiet and every part of it reports success: two stations on
different networks hear each other's announcements all day, each publishes a
`t:message` with no `d:`, the local bearers report `sent`, and neither ever sees
a word the other said. Why: a broadcast is an announcement, and shared transports
do not cross-forward announcements between their clients (section 12.12.1). The
answer is the archiver, as both destination and source:

- **Push what you publish to the archivers you chose.** Section 12.3 says a
  station pushes to the archivers its operator chose, 12.4 says when. Across the
  internet that push MUST be an addressed copy per chosen archiver (one directed
  message each), not a reliance on the broadcast reaching them. A `d:` wire is
  mail and excluded (sections 12.7, 12.8.1).
- **Pull what everybody else published from the same place.** A station's
  catch-up (section 12.10.1) MUST include its configured archivers, and
  implementations go wrong two ways: **asking only stations it can HEAR** (a
  neighbourhood, not the public everything, and a station with no radio in
  earshot needs the archiver MORE, not less); and **marking the ask
  `scope:local`** (section 9.11.3 says a `scope:local` packet is never carried,
  so the internet bearer refuses it before it leaves and the archiver across the
  world is listed, counted, never asked). A catch-up ask is DIRECTED: it reaches
  one named station that meters it (section 30.2), and needs no scope to stay
  cheap.
- **An always-on archiver is what makes this work at all** (section 12.9.4).
  Someone reachable over the internet must hold everything, or there is nowhere
  to push to or pull from. Stations with only radio neighbours are invisible to
  each other until one archiver they can all address is holding the conversation.
- **What an archiver admits off the internet.** Section 12.3's declaration rule
  guards against spooling other people's MAIL (the `d:` case) and MUST NOT be
  applied to publications: a `t:status` (section 27), a `t:reaction` (section
  7.5) and a `t:message` with no `d:` are written for everybody by definition. An
  archiver that refuses them is refusing to be an archiver of public traffic.

**Addressing is a prerequisite, and it fails silently.** All of the above is
directed traffic, so it depends on turning a callsign into the transport's own
address, and when that lookup fails an implementation naturally falls back to
broadcasting, the fallback being the failure dressed as a send. So a station
SHOULD publish its callsign as the display name on whatever identity the
transport announces; and a resolver MUST read every name the transport offers,
not the first it parses, since callsign-to-address for sending is the direction
that fails silently and so the one to check against the air.

**A station too small for a link layer still has a directed lane.** "Directed"
means addressed to one destination, not any particular mechanism. A
microcontroller with kilobytes free cannot carry a link layer or a
store-and-forward router and does not need one: an addressed, encrypted single
packet reaches a named destination through the same transports, forwarded by
destination like anything else, measured multi-hop across a public hub between
two networks. What it gives up is the receipt: silence and refusal look alike,
the right trade for an ask that repeats anyway (a poll, a gossip query, a replay
request, section 12.10.2), the wrong one for mail, which is why mail is held
under custody (section 12.7). So a constrained station's obligations shrink to
three: announce so paths exist, answer where it was asked from, and never assume
its packet arrived.

**Send on every lane that might work.** Where a transport offers more than one
addressed mechanism, a directed wire MAY go out on all of them: they fail
independently, and section 5's identifier collapses the duplicate into one record
on arrival. Measured on the bench: two phones on different networks exchanged one
addressed form while the other stayed silent between the same endpoints.
...
A listener uses `count:` to decide whether asking is worth it. Section 12.10.1's
pocket device cannot afford a `cmd:history` per period on the chance that
something arrived, a replay is metered (section 22.2), and one spent to be
...
section 12.11 will see it fall. What is required is that it MOVES when the
...
12.10.1's period remains the backstop whatever `count:` says or fails to say.

(`count:` also appears on `t:file kind:folder`, where it is the number of files
in a listing, section 7.7.3. The packet type says which is meant.)
...
lift, a spare battery and a berth are already in section 27 with a price and an
...
### 13.2 The other half of a mailbox

`serve:archive` is a station volunteering to hold mail. `t:mailbox` (section
9.12) is a recipient nominating. They are opposite directions of the same
...
**Sign it.** Signing is the default (section 7.1) and an unsigned service
...
should not be read (section 7.2) and set `scope:` on what should not travel
(section 9.11); neither depends on trusting the station that carries it.

### 13.4 One port on an IP network
...
`cmd:history` with the replay of section 33.2, so the socket adds no new
...
and the first station on a network knows nobody's. So `link:lan` (section 15.6)
...
Broadcast is heard by everyone at once, so relaying on it obeys section 6.2.1
...
| `input:` | repeater input, stated outright (section 14.4) |
| `tone:` | access tone |
...
### 14.1 Listening, or transmitting
...
everywhere else (section 11.1). Their absence says the station listens whenever
it is on, not that it never listens.

A recurring schedule is not expressed here. A weekly net is a `t:event`
(section 26) that names the frequency, and this packet describes the channel
itself rather than the calendar around it.

### 14.2 Recurring windows
...
### 14.4 Repeaters that listen elsewhere
...
### 14.6 Transmitting is regulated
...
not a claim of authority to use it, and section 7.4 continues to govern what may
be transmitted where.

### 14.7 Meeting on a working channel
...
Every field keeps its section 14 meaning, the keys say where, `until:` says
...
(section 15.6.1's bearer word) names it and `ch:` carries whatever label that
...
3. **Move.** On HEARING the acceptance the inviter tunes to the working
   ...
   custody or a mailbox like mail for any station that is away (section 6.3).
...
5. **Work, then give the channel back.** The exchange runs (section 11.2.2's
   ...
   killed mid-way is resumed later with `off:` (section 33.2) on whatever
...
- **The working channel is borrowed, never claimed.** Both stations return to
  ...
  vanished, and section 30 already says what a vanished station owes nobody.
- **One channel per invitation** (section 14.5). Offering a fallback is a
...
- **An unsigned invitation is not followed.** "Meet me elsewhere" is the
  ...
  unsigned mailbox declaration of section 9.12: ignored, not displayed.
- **Moving is transmitting.** Section 14.6 and 6.4 govern the working channel
...
This is also the missing handshake of section 11.2.2: when a pair's best bulk
...
A station reports in the unit it works in and says which it is (section 15.8).
...
131 bytes: 21.2 outside, 21.5 in the room, one timestamp, one transmission.
...
### 15.5 Telemetry and station type

| Key | Type | Meaning | Quantity |
|---|---|---|---|
| `batt` | `qty` | battery charge | proportion |
| `volt` | `qty` | supply voltage | voltage |
| `link` | `enum` | which bearer a reading is about (section 15.6) | |
| `busy` | `qty` | proportion of the last hour that bearer was occupied (section 15.6) | proportion |
| `txtime` | `qty` | proportion of the last hour this station transmitted | proportion |
| `hears` | `path` | callsigns heard directly, most relevant first (section 15.6.3) | |
| `peers` | `int` | how many stations are reachable in total (section 15.6.4) | |
| `mail` | `int` | messages held for other stations (section 15.6.5) | |
...
| `type` | `enum` | what the station is or is riding on, from the set in section 21.2 | |
| `fw` | `text` | the firmware version this station is running (section 11.8) | |
| `state` | `enum` | a device's principal condition, from the closed list in section 33.7 | |
| `level` | `qty` | how far, when a condition is partial (section 33.7) | proportion |
| `target` | `qty` | the setpoint a device holds a reading at (section 33.7) | |
...
wall clock, which a station reporting `epoch:` (section 15.7) does not have.
...
(section 9.3), they say what the sender believes about itself; a receiver
...
anomaly raises `t:warning kind:radiation` (section 24), which carries the
nine-relay budget an emergency deserves; the observation stream keeps its
ordinary three and its ordinary cadence (section 22.1).
...
reading takes a new key in this table and its unit family in section 15.9
...
### 15.6.1 A reading without a bearer is not a reading
...
The list is the one section 22.1 and `spectrum.md` already work through; this
key only gives it a name on the wire.

**Section 30.1 becomes computable because of this.** It already says a station
...
### 15.6.2 Why `busy:` matters more than it looks
...
for what is not. **`urg:` (section 9.5) decides what survives that decision**,
...
### 15.6.3 `hears:` is what makes a mesh diagnosable
...
9.12): the right mailbox is a station that other people list as heard.
...
The truncation of section 15.6.4 is **per bearer**, not a property of the
list. The advert channel cuts `hears:` to what fits one advert; the same
observation pushed to an archiver (section 12) carries the full list, over
section 7.6 parts when a busy gateway hears more than one packet holds,
...
### 15.6.4 `peers:` says how many were left out
...
### 15.6.5 `mail:` says there is something to collect
...
people** (section 6.4) and would hand over if asked. It is omitted when there
...
decimal degrees in WGS84 and no second option exists (section 15.1).

---

# Part IV. Posting and status

What a station broadcasts about itself and the world: status, polls, blog posts, passages, and reporting.
...
the group is an open name or a closed `X5` (section 10).

**Neither existing type would do.** A `t:blog` post is a document: it has a
`title:`, and a later post with the same title replaces it (section 29.1). Two
...
kept, and read later by people who were not, which is the distinction section 18
...
long one across up to nine parts (section 7.6), and `sig:` signs it. A status
...
and does not raise a notification; `urg:` (section 9.5) is the field that
speaks to the network, and `sev:` (section 24) is the one that speaks to
danger.

### 16.2 What a status is not

**Not carried.** Store-and-forward (section 6.4) delivers to a recipient, and a
...
**Not privileged.** Three relays, the default of section 6.1. A townhall post
...
10.7 describes for rosters, and buys nothing that a local list does not
already give.
...
at which anyone may say what the answer was. Section 30.3 makes that concrete:
...
`until:` (section 6.4), for the same reason, work with no deadline is work
nobody can ever stop doing.

### 17.2 Bounding who is being asked
...
|---|---|
| `d:` | one group, open or closed (section 10) |
| `scope:` | `local` for the bearers in range now, or ISO country codes (section 9.11) |
...
difference is the same one section 15.7 draws about rosters. Anybody in range
...
49 bytes each. Section 7.5 already says a reaction is **counted once per
...
6.3.1). `remove:vote` withdraws a vote without replacing it.
...
### 17.4 The count is local, and provisional
...
- **`cmd:history` improves a count** (section 33.2) and never completes it.

### 17.5 A poll is not a secret ballot
...
### 18.2 How long a post can be

A post is split across up to 9 parts like any other text (section 7.6), so its
...
`since:` and `until:` are the ordinary keys (section 11.1) and are not renamed
...
where section 10 gives a moderator `hide:` and everywhere else gives nobody
anything.
...
### 20.1 A report is a claim, never a verdict

**Nothing happens automatically because a report exists.** A report is not
moderation: `hide:` (section 15.3) is the only packet in this format that hides
...
Where a station is and what it needs: tracks, places, calls for help, warnings, notices, events, and offers and needs.
...
### 21.2 What the station is riding on
...
### 22.1 Naming, revising and withdrawing

`title:` is a `label` and works exactly as it does on a post (section 29.1): a
...
An `sos` is relayed up to nine times (section 6.1). It is never encrypted:
...
`watch` is a notice rather than a warning (section 25).
...
### 25.1 When the condition starts and ends
...
### 25.2 Withdrawing a notice or a warning
...
| `price:` | what is asked or offered, if any (section 16.1) |
...
Neither type is relayed further than ordinary traffic (section 6.1). A need is
...
### 27.3 When the price is not fixed
...
### 27.4 The format carries no money
...
Proving a callsign, spending airtime well, extending the format, and living alongside APRS.
...
A signature (section 7.1) proves the packet was written by the holder of a key.
...
### 29.1 Publishing a key

A station transmits `t:identity` (section 7.3) periodically, not only once,
...
`q:identity` (section 8) asks for one directly rather than waiting for the next
period.
...
### 29.5 What a failed challenge means
...
often, and what it owes the strangers who ask it for things. Sections 11.2 and
9.12 make that urgent: `cmd:history` and `cmd:file` let one station ask another
...
Section 9.7.2 says when to stop re-airing altogether: a retry is spent only
...
- **A bounded number of answers per period.** Section 29.4 already sets this
...
collapse on their identifiers (section 11.2.1), asking three stations with
...
this document later assigns it, the entry is added to the table in section 15.3
...
responsibility, subject to section 7.4. An `X1`, `X2` or `X3` callsign is generated
...
Worked examples, the cheat sheet, reserved words, and what is actually implemented.
...
### 34.1 Group conversation with a reply and a reaction
...
### 34.2 Direct message with both receipts
...
### 34.3 Request and partial answer
...
### 34.5 Clockless weather station anchored by a neighbour
...
with `t:receipt ... s:ack` **without** `q:ack` (section 9.7.1). Broadcasts,
...
A `t:channel` WITH `d:` is a working-channel invitation (section 14.7): meet
...
`cmd:set` makes a device state true (section 33.7): `state:` from the closed
...
A station is owned (section 11.9): unowned, it asks `q:owner` on local
...
and signed for by the controller (section 11.7.1).
...
- The station asking MUST notice when it stops moving. If a continuation comes
  ...
  stalled: that is not new traffic and must not be counted as any (12.10.2), and
...
Files, the whole flow (section 7.7): a listing file (`XFL1`, one `ref size
...
custody receipt. A file up to 896 bytes rides inline in `b:` over section 7.6
...
Archivers (section 12.9): content only from chosen depositors, NEVER from
...
`busy:` is what section 30 was missing: a duty cycle limits one transmitter and
...
(section 15.3) remains the only packet that hides anything. Mass false reporting
...
period (section 29.4's precedent). Refuse out loud with `code:429` and name
...
| Section 11.10 setting up a station (`ssid:` and `pass:` sealed, `wifi:`, `nick:`, `zone:`, `ap:`, `key:new`, `nsec:`; the claim and the ask carrying `k:`) | **implemented** on the ESP32 station (`common/xprs_app`: `cmdset_apply`, `setup_apply`, sealed bodies opened by `common/xprs_sig/xprsseal.c` after the signature and the allow-list), in the app core (a `$type:"secret"` field the host never stores, commands never deposited with an archiver, a station's key learned from its ask) and in the Firmwares wapp (`apps/firmwares`). Bench-validated on an ESP32-C3 and an Android phone over the station's hotspot: claim, stats, name and zone, a sealed network joined in a second; then, with the station on the network it was given, a new key followed through the restart. The Bluetooth leg was shown on a T-Dongle-S3 (2026-09-12): the phone's claim, name, stats and a sealed WiFi password each answered on the first airing at -49 dBm. The same station was set up from a desktop over the LAN, with a wrong password answered `500 m:wrong password` from the network the station went back to, and a `key:new` followed through the restart. The ESP32-C3 hears that phone at -96 to -100 dBm and decoded none of its frames; it is set up over its hotspot. A station that answers over the LAN also sends the answer straight to the address the command came from, because its broadcasts stopped reaching a phone on its hotspot once it had joined another network |
...
| Section 12.8, sealed-mail release to a hearing gateway | **specified, not implemented**; the nearest live relatives are the chat iGate mailbox (mail pulled from APRS-IS by an in-range station) and MeshStore custody, neither of which is driven by a published `hears:` claim |
| Section 12.9, `serve:archive` and the XDIR1 directory exchange | **specified, not implemented**; the philosophy already ships for files, `pointer_sync.dart` gossips signed ADDRESSES between file-archivers and re-verifies on merge, never copying content, but no station publishes a callsign directory or answers a miss with `m:try` |
| Section 6.2.1, redacted packets (`xr:`) | **implemented**: the composer parses `((...))`, derives the slow key and builds the bars and the `xr:` blob in the core (`XprsCrypto.redact/restore`, `hal_xprs_redact`/`hal_xprs_unlock`), and a reader opens one at a tap, the passphrases that worked being remembered in the profile's encrypted store. Chat composes a redacted message in a 1:1, a closed group and the Local room; the wire is addressed there and scoped (`scope:local`) here. What is not implemented is redaction of the FIELDS the table permits (`pos:`, telemetry, `title:`): only `m:` is marked today |
| Section 14.7, working-channel invitations | **specified, not implemented** as packets; the dance itself ships in binary for one pair of bearers, the WiFi-Direct negotiation (BLE subtype `0x57` ADVERT/REQ/OFFER, `docs/ble5.md`) coordinates exactly this move from the shared advert channel to a private fast lane, and 14.7 is that handshake generalised to every bearer, in text, signed |
...
| Section 9.7.1, receipts signed by default | **specified, not implemented.** Signing exists (section 7.1) and receipts do not use it yet, which leaves the forged-`s:ack` deletion described there open on any station that honours the section 8 carrier release. The Reticulum side is not exposed to it, its acknowledgement is a link, not an XPRS packet, but an XPRS-native carrier would be |
| Section 9.7.2, parking a retry with no evidence | **implemented** on the Reticulum side: a retry is spent only against a live path or a beacon heard in the last three minutes (`RnsService._peerReachable`), otherwise the entry parks without burning a rung and the copy stays held |
...
| `ts:` on messages | **implemented** on the chat wire since 0.4.38: a message now states when it was written, so one that waited in a mailbox no longer shows the minute it arrived. A packet with no `ts:` is still read (section 15.7) |
...
| The chat wapp's own moderation | **implemented, and by a different design**: NIP-72 rooms with a NOSTR kind-9078 op-log, authority from the room event's author rather than a group keypair, and no roster at all (`docs/chat-rooms.md`). Nothing in section 10 is built, and reconciling the two is not attempted here |
...
| `dose`, `lifedose`, `radon`, `rf`, `efield`, `mfield` radiation (section 15.5.1), `odometer` | specified; no shipping station has a sensor for any of them yet |
